...
def load_global_model_bundle():
    ...
    if isinstance(gm, dict) and "weights" in gm and "scaler" in gm:
        ...
        coef = np.array(gm[0]); intercept = np.array(gm[1])
        n_features = coef.shape[-1]
        mean = np.zeros(n_features); scale = np.ones(n_features)
...
def predict_disease(features):
    ...
    weights = bundle["weights"]; scaler = bundle["scaler"]
    coef = weights[0]; intercept = weights[1]

    expected = len(coef[0]) if isinstance(coef[0], list) else len(coef)
    ...
    if prediction == 1:
        ...
        risk = "Low Risk" if confidence >= 75 else "Very Low Risk"
